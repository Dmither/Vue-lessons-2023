...
```html
<button @click="count++">Increase</button><span>Count: {{count}}</span>
<button @click="handlerName">Run handler</button> <!-- метод визначено раніше-->
```
...
<button @click="(event) => warn(`Cannot send the form`, event)">Send</button>
```
...
```html
<p>Message: {{ message }}</p>
<input v-model="message" placeholder="Change me">
...
```html
<input type="checkbox" value="First" v-model="checkedValues">
<input type="checkbox" value="Second" v-model="checkedValues">
```
